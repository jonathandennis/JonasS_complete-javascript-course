...
elements.shopping.addEventListener('click', e => {
    const id = e.target.closest('.shopping__item').dataset.itemid; // closest method as we need to find the element that contains our id we want to find. dataset.itemid calls the data attribute we set in the markup
...
elements.recipe.addEventListener('click', e => {
    if (e.target.matches('.btn-decrease, .btn-decrease *')) {
        ...
        if (state.recipe.servings > 1) { // stops user from going below 1 serving when clicking
